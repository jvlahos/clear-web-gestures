...
$(document).ready(function(){
	...
	$(document).hammer({drag_min_distance: 0}).on('drag', '.mod', function(event){
			if (event.gesture.deltaY > 67 ) {
				...
				newItemTop.css('height', "67px");
			} else {
				//$(window).scrollTop(0);
				newItemTop.css('height', event.gesture.deltaY+"px");
				newItemTop.find('.item')
					.css('background-color', 'hsl(0, 100%, '+((event.gesture.deltaY)-10)+'%)')
					.css('border-color', 'hsl(0, 100%, '+((event.gesture.deltaY)-30)+'%)');
				newItemTop.find('input').val('Pull to Create Task');
				newItemTop.css('-webkit-transform', 'rotateX('+(90-(((event.gesture.deltaY)/67)*90))+'deg)');
			}
	...
	$(document).hammer({drag_block_vertical: false}).on('drag', '.item-mod', function(event){
		...
		if (!v.$itemMod.hasClass('done') && !v.$itemMod.hasClass('was-done')) {
			...
			if ( dragX > 55 ) {
				v.$itemMod
					.css('background-position-x', (dragX-55)+"px")
					.addClass('check');
		...
		if (!v.$itemMod.hasClass('done') && !v.$itemMod.hasClass('was-done')) {
			...
			if ( releasePoint > 55 ) {
			    ...
			    setTimeout( function(){
					...
					v.$itemMod
						.addClass('remove')
						.css('-webkit-transform', 'translate(0px,'+((afterCount-1)*67)+'px)');
				},200 );
		...
		//If item is done
			...
			if ( releasePoint > 55 ) {
			    ...
			    setTimeout( function(){
					...
					v.$itemMod
						.addClass('remove')
						.css('-webkit-transform', 'translate(0px,'+(-(beforeCount+1)*67)+'px)');
				}, 200);
	...
	// 			.attr('style','')
	// 			.find('.item').attr('style','');
	// 		itemModTemplate.clone().insertBefore(spacer);
	// 		$('.mod .item-mod.new input')
	// 			.focus()
	// 			.closest('.item-mod.new').removeClass('new');
